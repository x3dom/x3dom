...
/*
 * This incorporates the change in the fragment shader for pickMode == 2 (texture coordinates). It uses
 * the "B" channel for the lower-order 4 bits of U & V coordinates. These changes must be implemented together
 *
 * For Texture Coordinate pickmode (pickMode == 2), do the picking in floating point...
 * 16 is the range of 4 bits
 * 15 = 0x0f
 * 240 = 0xf0
 *	Bit operations are used when calculating the revised values of U & V for performance
 */
				if (pickMode == 2) {		// Convert extended precision bytes
					pickPos.x = (pickPos.x << 4) | (pickPos.z&0x0f);
					pickPos.y = (pickPos.y << 4) | ((pickPos.z&0xf0)>>4);
					pickPos.z = 0;
				}
            }
