...
x3dom.gfx_webgl = (function () {
	...
	g_shaders['fs-x3d-storm'] = {type: "fragment", data:
		...
		"vec3 directionalLight(int lightIdx, vec3 normal, vec3 eye){" +
		"	float spot = 1.0;" +
		"	float attentuation = 1.0;" +
		"	vec3 lightDirection = normalize(-light[lightIdx].direction);" +
		
			// Calculate Ambient, Specular and Diffuse Color
		"	vec3 ambient  = light[lightIdx].ambientIntensity * material.diffuseColor * material.ambientIntensity;" +
		"	vec3 diffuse  = light[lightIdx].intensity * material.diffuseColor * fragColor * x3dDot(normal, lightDirection);" +
		...
		"vec3 pointLight(int lightIdx, vec3 normal, vec3 eye){" +
		"	float spot = 1.0;" +
		...
		"	vec3 ambient  = light[lightIdx].ambientIntensity * material.diffuseColor * material.ambientIntensity;" +
		"	vec3 diffuse  = light[lightIdx].intensity * material.diffuseColor * fragColor * x3dDot(normal, lightDirection);" +
		"	vec3 specular = light[lightIdx].intensity * material.specularColor * pow(x3dDot(normal, normalize(lightDirection+eye)), material.shininess * 128.0);" +
		...
		"vec3 spotLight(int lightIdx, vec3 normal, vec3 eye){" +
		...
		"		if(light[i].type == 0) rgb += pointLight(i, normal, eye);" +
		"		else if(light[i].type == 1) rgb += directionalLight(i, normal, eye);" +
		"		else if(light[i].type == 2) rgb += spotLight(i, normal, eye);" +
		"	}" +
		"	if(useTexture) { " +
		"		if(numLights == 0) rgb = vec3(1.0);" +
		"		rgb = texture2D(tex, fragTexcoord).rgb * rgb;" +
		"		rgb = fog.color * (1.0-f0) + f0 * (material.emissiveColor + rgb);" +
		"	}else{" +
		"		if(numLights == 0 && useMaterial) rgb = material.diffuseColor;" +
		"		if(numLights == 0 && !useMaterial) rgb = fragColor;" +
		"		rgb = fog.color * (1.0-f0) + f0 * (material.emissiveColor + rgb);" +
		"	}" +
		"   gl_FragColor = vec4(rgb, material.transparancy);" +
		"}"
		};
	...
	function wrapShaderProgram(gl, sp) 
	{
		...
		for (i=0; i < numUniforms; ++i) {
			...
			//x3dom.debug.logInfo(gl.BOOL + " - " + obj.type + " - " + obj.name);
			switch (obj.type) {
				case gl.SAMPLER_2D:
	...
	Context.prototype.setupShape = function (gl, shape) 
	{
		...
		else 
		{
            ...
            if (x3dom.isa(shape._cf.geometry.node, x3dom.nodeTypes.PointSet)) {
                ...
                if (shape._webgl.colors.length) {
					x3dom.debug.logInfo("3");
                   // shape._webgl.shader = getShaderProgram(gl, 
                   //                       ['vs-x3d-vertexcolorUnlit', 'fs-x3d-vertexcolorUnlit']);
					shape._webgl.shader = getShaderProgram(gl, ['vs-x3d-storm', 'fs-x3d-storm']);
                }
                else {
					x3dom.debug.logInfo("4");
                   // shape._webgl.shader = getShaderProgram(gl, 
                    //                      ['vs-x3d-default', 'fs-x3d-default']);
					shape._webgl.shader = getShaderProgram(gl, ['vs-x3d-storm', 'fs-x3d-storm']);
                }
	...
	Context.prototype.renderScene = function (scene, pick) 
	{
		...
		for (i=0, n=zPos.length; i<n; i++)
		{
			...
			var min = x3dom.fields.SFVec3f.MAX();
            var max = x3dom.fields.SFVec3f.MIN();
			shape._cf.geometry.node._mesh.getBBox(min, max, false);
			min = obj[1]._cf.geometry.node._mesh._min;
			max = obj[1]._cf.geometry.node._mesh._max;
			
			scene.getViewpoint().getFrustum(mat_scene.mult(transform));
			if(scene.getViewpoint().frustumHitAABB(min, max) > 0){
				...
				if (mat) {
					...
					sp['material.transparancy'] 		= 1.0 - mat._vf.transparency;
				}
				...
				if (numLights > 0)
				{
					...
					for(var p=0; p<numLights; p++){
						if(x3dom.isa(slights[p], x3dom.nodeTypes.PointLight))
						{
							...
							sp['light[' + p + '].type']				= 0;
							sp['light[' + p + '].on']				= slights[p]._vf.on;
				...
				// Set HeadLight
				//===========================================================================
				var nav = scene.getNavInfo();
				if(nav._vf.headlight){
					sp.numLights = numLights+1;
					sp['light[' + numLights + '].type']				= 1;
					sp['light[' + numLights + '].on']				= true;
								...
								cnt < shape._webgl.texture.length; cnt++)
				{
				  if (shape._webgl.texture[cnt])
				  {
					...
					if (shape._cf.appearance.node._cf.textureTransform.node !== null)
					{
						...
						sp.texTrafoMatrix = x3dom.fields.SFMatrix4f.identity();
					}
