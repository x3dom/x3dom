...
var X3DNode = defineClass(null,
    ...
    },
    {
        ...
        _attribute_SFRotation: function (ctx, name, x, y, z, a) {
            this['_'+name] = ctx.xmlNode.hasAttribute(name) ? SFQuat.parseAxisAngle(ctx.xmlNode.getAttribute(name)) : new SFQuat(x, y, z, a);
        },
...
var OrientationInterpolator = defineClass(X3DInterpolatorNode,
    function (ctx) {
        ...
        if (ctx.xmlNode.hasAttribute('keyValue'))
            this._keyValue = Array.map(ctx.xmlNode.getAttribute('keyValue').split(/\s*,\s*/), SFQuat.parseAxisAngle);
        else
            this._keyValue = [];
...
var Scene = defineClass(X3DGroupingNode,
    ...
    },
    {
        _getViewpointMatrix: function () {
            ...
            var rot = SFQuat.axisAngle(rightwards.cross(upwards).normalised(), -this.elevation). // XXX: this is all wrong
...
function X3DDocument(canvas, ctx, env) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.env = env;
